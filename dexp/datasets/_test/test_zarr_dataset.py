...
def test_zarr_dataset_livecycle(tmp_path: Path):
    zdataset = ZDataset(path=tmp_path / "test.zarr", mode="w", store="dir")
...
def test_zarr_integrity(tmp_path: Path):
    zdataset = ZDataset(path=tmp_path / "test.zarr", mode="w", store="dir")

    array = zdataset.add_channel(
        name="first", shape=(10, 100, 100, 100), chunks=(1, 50, 50, 50), dtype="f4", codec="zstd", clevel=3
    )
...
def test_add_channels_to(tmp_path: Path):
    dataset1_path = tmp_path / "test1.zarr"
    zdataset1 = ZDataset(path=dataset1_path, mode="w", store="dir")

    array1 = zdataset1.add_channel(
        name="first", shape=(10, 11, 12, 13), chunks=None, dtype="f4", codec="zstd", clevel=3
    )

    array1[...] = 1

    dataset2_path = tmp_path / "test2.zarr"
    zdataset2 = ZDataset(path=dataset2_path, mode="w", store="dir")

    array2 = zdataset2.add_channel(
        name="second", shape=(17, 10, 20, 30), chunks=None, dtype="f4", codec="zstd", clevel=3
    )

    array2[...] = 1

    zdataset2.add_channels_to(
        str(dataset1_path),
        channels=("second",),
        rename=("second-",),
    )

    zdataset1_reloaded = ZDataset(path=dataset1_path, mode="r", store="dir")
...
def test_zarr_cli_history(tmp_path: Path):
    source_path = tmp_path / "test.zarr"
    ...
    first_copy = tmp_path / "first.zarr.zip"
    first_command = f"dexp copy {source_path} -o {first_copy}"
    os.system(first_command)

    second_copy = tmp_path / "second.zarr.zip"
...
def test_zarr_parent_metadata(tmp_path: Path):
    source_path = tmp_path / "test.zarr"
    dataset = ZDataset(path=source_path, mode="w", store="dir")

    dataset.append_metadata({"dz": 4.0, "Channel1": {"dt": 10.0}})
    parent_metadata = dataset.get_metadata()
    parent_metadata.pop("cli_history")

    child_dataset = ZDataset(path=tmp_path / "child.zarr", mode="w", parent=dataset)
